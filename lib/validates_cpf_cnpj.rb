    ...
    class CpfOrCnpjValidator < ActiveModel::EachValidator
      ...
      def validate_each(record, attr_name, value)
        ...
        if value.to_s.gsub(/[^0-9]/, '').length <= 11
          if (not value.to_s.match(/\A\d{11}\z/) \
            and not value.to_s.match(/\A\d{3}\.\d{3}\.\d{3}\-\d{2}\z/)) \
            or Cpf.blacklisted?(value, options.fetch(:blacklist, true)) \
            or not Cpf.valid?(value)

            record.errors.add(attr_name)
          end
        else
          if (not value.to_s.match(/\A\d{14}\z/) \
            and not value.to_s.match(/\A\d{2}\.\d{3}\.\d{3}\/\d{4}\-\d{2}\z/)) \
            or not Cnpj.valid?(value)
